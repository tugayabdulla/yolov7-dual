...
class AVG(nn.Module):
    ...
    def forward(self, rgb_features, thermal_features):
        return (rgb_features + thermal_features) / 2
    ...
    def forward_once_new(self, x, profile=False):
        ...
        for m in self.model[1]:
            ...
            if profile:
                ...
                print('%10.1f%10.0f%10.1fms %-40s' % (o, m.np, dt[-1], m.type))
            x = m(x)  # run
            y.append(x) # if m.i in self.save["thermal"] else None)  tugaytodo
        thermal_y = y
        thermal_dt = dt
        thermal_last_x = x
        ...
        for m in self.model[-1]:
            ...
            y.append(x) # if m.i in self.save["head"] else None)  # save output tugaytodo
...
def parse_model_parts(part, ch, d):
    ...
    for i, (f, n, m, args) in enumerate(d[part]):  # from, number, module, args
        ...
        if f != -1 and isinstance(f, int) and f > 0 and f < len_ch:
            c2_ = ch[f]
            fuse_layer = ChannelAttentionModule(c2_)
...
def parse_model_new(d, ch):
    ...
    last_fusion = ChannelAttentionModule(ch[-1])
