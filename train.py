...
def train(hyp, opt, device, tb_writer=None):
    ...
    if not weights.endswith('.pt'):  
        if opt.rgb_weights is not None:
            opt.weights = opt.rgb_weights
        else:
            pass
    ...
    for k, v in model.named_parameters():
        ...
        if any(x in k for x in freeze) or "model.0." in k or "model.5." in k or "model.1." in k:
            print('freezing %s' % k)
    ...
    nl = model.model[-1][-1].nl  # number of detection layers (used for scaling hyp['obj'])
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (rgb_images, thermal_images, targets, rgb_path, thermal_path, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 6) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], rgb_images.shape[-1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rgb-weights',type=str,  help='initial thermal weights path')
